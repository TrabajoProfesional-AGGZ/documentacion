...
Dada la arquitectura descentralizada basada en microservicios, el control del código fuente de SocioUnido se estructuró a través de un esquema multirepositorio alojado en la organización privada del equipo, compuesto por los siguientes repositorios clave para el producto mínimo viable:

\begin{itemize}
    \item \textbf{\texttt{plataforma-web}}: Repositorio del panel web administrativo de escritorio diseñado para la tesorería del club, desarrollado en React con Vite.
    \item \textbf{\texttt{aplicacion}}: Repositorio de la aplicación web progresiva (\textit{PWA}) adaptativa de autogestión para los socios y empleados de control.
    \item \textbf{\texttt{gateway}}: Código declarativo de configuración y enrutamiento perimetral de la puerta de enlace de aplicaciones basada en el motor de alto rendimiento KrakenD.
    \item \textbf{\texttt{healthchecker}}: Repositorio del sensor de observabilidad e interactividad para el monitoreo de nodos y conciliación contable de tarifas comerciales.
    \item \texttt{\textbf{ms-club}} / \texttt{\textbf{ms-pagos}} / \texttt{\textbf{ms-acceso}} / \texttt{\textbf{ms-autenticacion}} / \texttt{\textbf{ms-analiticas}} / \texttt{\textbf{ms-bot-conversacional}}: Repositorios completamente independientes y desacoplados correspondientes al código de cada microservicio del ecosistema.
    \item \textbf{\texttt{scrum}}: Repositorio que documenta y audita de manera pública las métricas de velocidad, burndown, burnup y las actas de las 13 iteraciones semanales del ciclo ágil.
    \item \textbf{\texttt{bitacora}}: Repositorio público que consolida los reportes puntuales de avance de cada una de las 37 semanas cronológicas de duración del Trabajo Profesional.
    \item \textbf{\texttt{.github}}: Repositorio que actúa como la torre de control y visualización unificada del proyecto, exponiendo a través de un README central dinámico el estado de salud, badges y reportes consolidados de calidad de cada componente del ecosistema.
\end{itemize}
...
\begin{itemize}
    \item Las interfaces de usuario de *frontend* (\texttt{plataforma-web} y \texttt{aplicacion}) se compilaban y alojaban de manera directa sobre la plataforma de computación elástica \textit{Vercel}, aprovechando su alta optimización para aplicaciones React.
    \item Los componentes de *backend* (\textit{API Gateway} y la suite de microservicios) se empaquetaban en contenedores de infraestructura ligera mediante imágenes Docker y se desplegaban de manera ininterrumpida sobre la plataforma de servicios en la nube \textit{Render}.
\end{itemize}
